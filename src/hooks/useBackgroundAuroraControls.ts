...
export function useBackgroundAuroraControls(activeView: ActiveView) {
  ...
  const [values, set] = useControls(() => ({
    'Background Aurora': folder({
      ...
      bgOpacity: { value: 0.3, min: 0, max: 1, step: 0.05, label: 'Opacity' },
      bgBlur: { value: 80, min: 20, max: 200, step: 5, label: 'Blur (px)' },
      mouseFollow: { value: 0.5, min: 0, max: 1, step: 0.05, label: 'Mouse Follow' },
    }, { render: () => isAurora }),
  }), [activeView]);
